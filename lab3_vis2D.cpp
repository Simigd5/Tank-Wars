...
void Lab3_Vis2D::OnInputUpdate(float deltaTime, int mods)
{
    ...
    // Miscarea ferestrei logice cu W, A, S, D
    if (window->KeyHold(GLFW_KEY_W)) {
        logicSpace.y += moveSpeed * deltaTime;  // Translatie in sus
    }
    if (window->KeyHold(GLFW_KEY_S)) {
        logicSpace.y -= moveSpeed * deltaTime;  // Translatie in jos
    }
    if (window->KeyHold(GLFW_KEY_A)) {
        logicSpace.x -= moveSpeed * deltaTime;  // Translatie la stanga
    }
    if (window->KeyHold(GLFW_KEY_D)) {
        logicSpace.x += moveSpeed * deltaTime;  // Translatie la dreapta
    }
    ...
    if (window->KeyHold(GLFW_KEY_Z)) {
        logicSpace.width -= zoomSpeed * deltaTime;
        logicSpace.height -= zoomSpeed * deltaTime;

        // Prevenim ca dimensiunile sa devina negative sau prea mici
        if (logicSpace.width < 0.1f) {
            logicSpace.width = 0.1f;
        }
        if (logicSpace.height < 0.1f) {
